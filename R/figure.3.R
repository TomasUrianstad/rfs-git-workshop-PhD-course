...
#make ggplot 
plot <- ggplot(aes(timepoint, vo2.kg, colour = sex, group = id), data = d) +
  ...
  geom_point(data = mean, aes(timepoint, mean, group = sex, color = sex),
             position = position_dodge(width = 1.5),
             size = 2.5) +
  geom_linerange(data = mean, aes(timepoint, mean, group = sex, colour = sex, ymin = mean - sd, ymax = mean + sd),
                 position = position_dodge(width = 1.5),
                 size = 1) +
  ...
  scale_color_manual(values = c("f" = "deeppink", "m" = "deepskyblue"),
                     name = "Sex", 
